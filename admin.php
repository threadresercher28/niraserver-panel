...
// Подключение к БД через константы из config.php
try {
    $pdo = new PDO(
...
@media (max-width: 768px) {
    ...
    .header-action-btn { padding: 4px 10px; font-size: 0.75rem; }
}
</style>
</head>
<body>
        ...
        <?php elseif ($section == 'groups'): ?>
            <?php if (isset($_GET['action']) && $_GET['action'] == 'edit' && $edit_group): ?>
                <div class="table-container" style="padding:20px;">
                    ...
                    <form method="POST"><input type="hidden" name="csrf_token" value="<?= $csrf_token ?>"><input type="hidden" name="group_action" value="rename"><input type="hidden" name="old_name" value="<?= safe_html($edit_group) ?>">
...
<div id="massChannelModal" class="modal">
    <div class="modal-content">
        <div class="modal-header"><h3>Импорт каналов</h3><button type="button" class="btn btn-secondary" onclick="closeModal('massChannelModal')">&times;</button></div>
        <form method="POST" id="massChannelForm">
            <div class="modal-body">
                <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
                <input type="hidden" name="channel_action" value="mass_add">
                <input type="hidden" name="channels_data" id="channels_data">

                <div class="form-group">
                    <label>Ручной ввод (формат: channel_id|channel_name|channel_class|channel_group|icon_url|stream_url)</label>
                    <textarea class="form-control" id="manual_data" rows="6" placeholder="channel_1|Первый канал||Группа 1||http://example.com/stream.m3u8"></textarea>
                    <button type="button" class="btn btn-primary" id="fillFromManualBtn" style="margin-top:8px;"><i class="fas fa-arrow-down"></i> Заполнить из ручного ввода</button>
                </div>

                <hr style="border-color:#2a2e36; margin:15px 0;">

                <div class="form-group">
                    <label>Загрузить M3U8 файл</label>
                    <input type="file" id="m3uFile" accept=".m3u,.m3u8,audio/x-mpegurl,application/vnd.apple.mpegurl" class="form-control" style="padding:5px;">
                    <button type="button" class="btn btn-primary" id="parseM3UBtn" style="margin-top:10px;"><i class="fas fa-upload"></i> Парсить и добавить</button>
                </div>
                ...
                <div class="form-group">
                    <label style="display:flex; align-items:center; gap:10px;">
                        <input type="checkbox" id="ignoreGroupsCheckbox"> Не учитывать группы (все каналы будут без группы)
                    </label>
                </div>

                <div class="form-group">
                    <label>Данные для импорта (редактируемые)</label>
                    <textarea class="form-control" id="channels_data_display" rows="10" placeholder="Данные появятся здесь после ручного ввода или парсинга файла"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeModal('massChannelModal')">Отмена</button>
                <button type="submit" class="btn btn-primary" id="massAddSubmit">Импортировать</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('click', (e) => { if (!sidebar.contains(e.target) && !toggleBtn.contains(e.target) && sidebar.classList.contains('open')) sidebar.classList.remove('open'); });

function openChannelModal(channel = null) {
    const modal = document.getElementById('channelModal');
    if (channel) {
        document.getElementById('channelModalTitle').innerText = 'Редактировать канал';
        document.getElementById('channelId').value = channel.channel_id;
        document.getElementById('channel_id').value = channel.channel_id;
        document.getElementById('channel_name').value = channel.channel_name;
        document.getElementById('channel_class').value = channel.channel_class || '';
        document.getElementById('channel_group').value = channel.channel_group || '';
        document.getElementById('icon_url').value = channel.icon_url || '';
        document.getElementById('stream_url').value = channel.stream_url;
    } else {
        document.getElementById('channelModalTitle').innerText = 'Добавить канал';
...
function openKeyModal(key = null) {
    ...
    if (key) {
        ...
        document.getElementById('edit_key').value = key.access_key;
        document.getElementById('access_key').value = key.access_key;
        document.getElementById('access_key').readOnly = true;
        document.getElementById('discription').value = key.discription || '';
        document.getElementById('key_status').value = key.status;
    } else {
        document.getElementById('keyModalTitle').innerText = 'Добавить ключ';
        document.getElementById('keyForm').reset();
        document.getElementById('edit_key').value = '';
        document.getElementById('access_key').readOnly = false;
    }
    modal.classList.add('active');
}
...
function filterChannels() {
    const s = document.getElementById('searchChannels')?.value.toLowerCase() || '';
    ...
    document.querySelectorAll('#channelsTableBody tr').forEach(r => {
        ...
        const i = r.getAttribute('data-id')?.toLowerCase() || '';
        const gr = r.getAttribute('data-group') || '';
        r.style.display = (s && !n.includes(s) && !i.includes(s)) || (g && gr !== g) ? 'none' : '';
    });
}
function filterKeys() {
    const s = document.getElementById('searchKeys')?.value.toLowerCase() || '';
    document.querySelectorAll('#keysTableBody tr').forEach(r => {
        const k = r.getAttribute('data-key')?.toLowerCase() || '';
        const d = r.getAttribute('data-desc')?.toLowerCase() || '';
        r.style.display = (s && !k.includes(s) && !d.includes(s)) ? 'none' : '';
    });
}
function filterGroups() {
    const s = document.getElementById('searchGroups')?.value.toLowerCase() || '';
    document.querySelectorAll('#groupsTableBody tr').forEach(r => {
        const g = r.getAttribute('data-group')?.toLowerCase() || '';
        r.style.display = (s && !g.includes(s)) ? 'none' : '';
    });
}
...
function parseM3U(content) {
    ...
    while (i < lines.length) {
        ...
        if (line.startsWith('#EXTINF')) {
            let channelName = '', channelGroup = '', iconUrl = '';
            const extinf = line.substring(7);
            const groupMatch = extinf.match(/group-title="([^"]*)"/);
            if (groupMatch) channelGroup = groupMatch[1];
            const logoMatch = extinf.match(/tvg-logo="([^"]*)"/);
            if (logoMatch) iconUrl = logoMatch[1];
            const commaPos = extinf.lastIndexOf(',');
            channelName = commaPos !== -1 ? extinf.substring(commaPos + 1).trim() : 'Unknown';
            i++;
            while (i < lines.length && lines[i].trim() === '') i++;
            if (i < lines.length) {
                let url = lines[i].trim();
                while (url.startsWith('#') && i < lines.length) { i++; url = lines[i]?.trim() || ''; }
                if (url && !url.startsWith('#')) {
                    let id = channelName.toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
                    if (!id) id = 'channel_' + (channels.length+1);
                    channels.push({ channel_id: id, channel_name: channelName, channel_class: '', channel_group: channelGroup, icon_url: iconUrl, stream_url: url });
                }
            }
        }
        i++;
...
document.getElementById('fillFromManualBtn')?.addEventListener('click', () => {
    const manual = document.getElementById('manual_data').value;
    if (manual.trim()) {
        document.getElementById('channels_data_display').value = manual;
        updateHiddenData();
        alert('Данные из ручного ввода скопированы в поле для импорта.');
    } else {
        alert('Введите данные в поле ручного ввода');
    }
});

document.getElementById('parseM3UBtn')?.addEventListener('click', () => {
    const fileInput = document.getElementById('m3uFile');
    if (!fileInput.files.length) { alert('Выберите M3U8 файл'); return; }
    const file = fileInput.files[0];
    const reader = new FileReader();
    reader.onload = function(e) {
        const channels = parseM3U(e.target.result);
        if (channels.length === 0) { alert('Не удалось распознать каналы в файле'); return; }
        const lines = channels.map(ch => `${ch.channel_id}|${ch.channel_name}|${ch.channel_class}|${ch.channel_group}|${ch.icon_url}|${ch.stream_url}`);
        const current = document.getElementById('channels_data_display').value;
        const newData = current.trim() ? current + '\n' + lines.join('\n') : lines.join('\n');
        document.getElementById('channels_data_display').value = newData;
        updateHiddenData();
        alert(`Распознано ${channels.length} каналов. Добавлены в поле для импорта.`);
    };
...
function updateHiddenData() {
    const displayData = document.getElementById('channels_data_display').value;
    document.getElementById('channels_data').value = displayData;
}

document.getElementById('massChannelForm')?.addEventListener('submit', function(e) {
    const ignoreGroups = document.getElementById('ignoreGroupsCheckbox').checked;
    let data = document.getElementById('channels_data_display').value;
    if (ignoreGroups && data.trim()) {
        ...
        data = newLines.join('\n');
        document.getElementById('channels_data_display').value = data;
        updateHiddenData();
    }
    updateHiddenData();
});
...
if (urlGroup && document.getElementById('groupFilter')) { document.getElementById('groupFilter').value = urlGroup; filterChannels(); }
